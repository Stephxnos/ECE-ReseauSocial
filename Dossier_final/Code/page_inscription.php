...
if(isset($_POST['forminscription']))
{
	...
	if(!empty($_POST['nom']) AND !empty($_POST['pseudo']) AND !empty($_POST['age']) AND !empty($_POST['mdp']))
	{
		...
		$sql="SELECT `Pseudo` FROM `bdd.sql`.`UTILISATEUR` WHERE";
		$sql.="`Pseudo`= '$pseudo'";
		$query = $bdd->query($sql); 
		...
		{ 
			$admindefaut=0;
			$sql = "INSERT INTO `bdd.sql`.`UTILISATEUR` (`Pseudo`, `IDUTILISATEUR`, `AGE`, `MDP`, `SEXE` VALUES(";
			...
			$sql2="SELECT * FROM `bdd.sql`.`UTILISATEUR` WHERE";
			$sql2.="`Pseudo`= '$pseudo'";
